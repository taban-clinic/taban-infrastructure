...
check "upload removed from incoming after a successful deploy" test ! -e "$a1" -a ! -e "$a1.sha256"
...
check "rolled-back upload kept for inspection" test -f "$a2" -a -f "$a2.sha256"
...
check "refused upload kept for inspection" test -f "$a3"
a4="$(make_artifact demo v1.1.2 ddddddd 200)"
echo 0000 >"$a4.sha256"
check "checksum mismatch refused" expect_exit 1 "$bin/deploy-release" deploy demo "$a4"
a1="$(make_artifact demo v1.0.0 aaaaaaa 200)"  # re-upload: the first copy was cleaned up
...
check "upload of a refused redeploy kept" test -f "$a1"
